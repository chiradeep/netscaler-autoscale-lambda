...
lambda_role_arn=$(aws iam list-roles  | jq -r '.Roles[] | select(.RoleName == "$LAMBDA_ROLE_NAME") | .Arn')
echo "Lambda Role ARN is $lambda_role_arn"
